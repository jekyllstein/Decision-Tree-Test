...
include("decisionTreeTypesv2.jl")
include("decisionTreeUtilsv2.jl")
include("inputSplitFunctionsv2.jl")
...
function makeRegTree(input::Vector{T}, output::Vector{T}, sqOutput::Vector{T}, selectedInd::Vector{Int64}; p = 5, q = 1) where T <: Real 
	...
	if length(selectedInd) <= p
		...
		if (S - minValue) < q
			...
			DecisionNode(leaf, 1, splitPoint, makeTree(input, output, sqOutput, ind1, p = p, q = q), makeTree(input, output, sqOutput, ind2, p = p, q = q))
		end
	end
end
...
function makeDecisionTree(input::vectorInputType, output::DenseVector{Bool}; inputSortInds::Vector{Vector{Int64}} = [sortperm(v) for v in input], inputInvSortInds::Vector{Vector{Int64}} = [sortperm(v) for v in inputSortInds], selectedInd::AbstractVector{Bool} = fill(true, length(output)), p = 10)
	orderedInd = find(selectedInd)
	...
	if (length(orderedInd) <= p) | (E == 0)
		...
		if (count(ind1) < p) | (count(ind2) < p) | (minValue == E)
			...
			if typeof(splitCond) == Set{Int64}
				DecisionLabelNode(leaf, minInd, splitCond, makeDecisionTree(input, output, inputSortInds = inputSortInds, inputInvSortInds = inputInvSortInds, selectedInd = ind1, p = p), makeDecisionTree(input, output, inputSortInds = inputSortInds, inputInvSortInds = inputInvSortInds, selectedInd = ind2, p = p))
			else
				DecisionNode(leaf, minInd, splitCond, makeDecisionTree(input, output, inputSortInds = inputSortInds, inputInvSortInds = inputInvSortInds, selectedInd = ind1, p = p), makeDecisionTree(input, output, inputSortInds = inputSortInds, inputInvSortInds = inputInvSortInds, selectedInd = ind2, p = p))
			end
		end
	end
end
...
function makeDecisionTree(input::Vector{T}, output::U, sqOutput::U, selectedInd::V = collect(eachindex(output)); p = 10, t = 1e-4, sortedInd = [eltype(v) <: AbstractFloat ? sortperm(v) : Vector{Int64}() for v in input], tmp::Vector{Int64} = fill(0, length(output))) where T <: Any where U <: outputNumType where V <: Any
	pred = 0.0
	c = 0
	sqTmp = 0.0
	...
	E = (sqMean - pred^2)*length(selectedInd)
	leaf = DecisionLeaf(selectedInd, pred, typeof(pred)(E/length(selectedInd)))
	if (length(selectedInd) <= p) | (E == 0)
		leaf
	else
		...
		splitOut = [typeof(input[i][1]) <: AbstractFloat ? findRegSplit(Vector{typeof(input[i][1])}(input[i]), output, sqOutput, selectedInd, sortedInd[i], tmp) : findRegSplit(Vector{typeof(input[i][1])}(input[i]), output, sqOutput, selectedInd) for i in 1:length(input)]
		minValues = [a[4] for a in splitOut]
		minSplitSize = [min(a[5], a[6]) for a in splitOut]
		validInd = findall(minSplitSize .>= p)
		# println(minSplitSize)
		if isempty(validInd)
			...
			# (leftPartition, ind1, ind2, minValue) = findBoolLabelSplit(input, output, selectedInd)
			# println(string("Ind1 length = ", length(ind1)))
			# println(string("Ind2 length = ", length(ind2)))
			if ((E - minValue)/E < t)
				# println("Best split of valid size has error reduction less than threshold, creating leaf")
				leaf
			else
				if typeof(splitCond) == Set{Int64}
					DecisionLabelNode(leaf, minInd, splitCond, makeDecisionTree(input, output, sqOutput, ind1, p = p, t = t, sortedInd = sortedInd, tmp = tmp), makeDecisionTree(input, output, sqOutput, ind2, p = p, t = t, sortedInd = sortedInd, tmp = tmp))
				else
					DecisionNode(leaf, minInd, splitCond, makeDecisionTree(input, output, sqOutput, ind1, p = p, t = t, sortedInd = sortedInd, tmp = tmp), makeDecisionTree(input, output, sqOutput, ind2, p = p, t = t, sortedInd = sortedInd, tmp = tmp))
				end
			end
		end
	end
...
function makeTestDecisionTree(input::Vector{T}, output::AbstractArray{U}, sqOutput::AbstractArray{U}, testInput::Vector{T}, testOutput::AbstractArray{U}, sqTestOutput::AbstractArray{U}; selectedInd::V = eachindex(output), testSelectedInd::V = eachindex(testOutput), p = 10, t = 1e-4, sortedInd = [eltype(v) <: AbstractFloat ? sortperm(v) : Vector{Int64}() for v in input], tmp::Vector{Int64} = fill(0, length(output))) where T <: Any where U <: Real where V <: AbstractArray{Int64, 1}
	...
	if (length(selectedInd) <= p) | (E == 0)
		...
		splitOut = map(i -> typeof(input[i][1]) <: AbstractFloat ? findRegSplit(input[i], output, sqOutput, selectedInd, sortedInd[i], tmp) : findRegSplit(input[i], output, sqOutput, selectedInd), 1:length(input))
		# println("hi")
			...
			if (E - minValue)/E < t
				...
				if typeof(splitCond) == Set{Int64}
					(testInd1, testInd2) = if isempty(testSelectedInd)
						...
						testInd1 = testSelectedInd[find(testSplits)]
						testInd2 = testSelectedInd[find(a -> !a, testSplits)]
						(testInd1, testInd2)
					end
...
function makeDecisionTree(input::T, output::U, selectedInd::V = eachindex(output); p = 10) where T <: vectorNullInputType where U <: DenseArray{Bool, 1} where V <: AbstractArray{Int64, 1}
	E = calcBoolEntropy(output[selectedInd])
	pred = mean(output[selectedInd])
	leaf = DecisionLeaf(collect(selectedInd), pred, typeof(pred)(E))
	if (length(selectedInd) <= p) | (E == 0)
...
function makeRandomSubspaceTree(input::T, output::U, selectedInd::V = eachindex(output); p = 10) where T <: vectorNullInputType where U <: DenseArray{Bool, 1} where V <: AbstractArray{Int64, 1}
	f = length(input) #number of feature vectors
	n = floor(Int64, sqrt(f)) #number of candidate features at each split
	E = calcBoolEntropy(output[selectedInd])
	pred = mean(output[selectedInd])
...
function pruneTree(tree::Union{DecisionLeaf, DecisionTestTrainLeaf}, path::Vector{Bool}, pathCount=1)
	tree
end

function pruneTree(tree::DecisionNode, path::Vector{Bool}, pathCount=1)
#generate a pruned tree by condensing the branch reached by following "path" into a leaf
	...
	elseif !path[pathCount]
		DecisionNode(tree.leaf, tree.splitCol, tree.splitPoint, pruneTree(tree.left, path, pathCount+1), tree.right)
	else
		DecisionNode(tree.leaf, tree.splitCol, tree.splitPoint, tree.left, pruneTree(tree.right, path, pathCount+1))
	end
end

function pruneTree(tree::DecisionTestTrainNode, path::Vector{Bool}, pathCount=1)
#generate a pruned tree by condensing the branch reached by following "path" into a leaf
	if pathCount == length(path)
		DecisionTestTrainLeaf(tree.leaf.ind, tree.leaf.prediction, tree.leaf.err, tree.leaf.testInd, tree.leaf.testErr)
	elseif !path[pathCount]
		DecisionTestTrainNode(tree.leaf, tree.splitCol, tree.splitPoint, pruneTree(tree.left, path, pathCount+1), tree.right)
	else
		DecisionTestTrainNode(tree.leaf, tree.splitCol, tree.splitPoint, tree.left, pruneTree(tree.right, path, pathCount+1))
	end
end

function pruneTree(tree::DecisionLabelNode, path::Vector{Bool}, pathCount=1)
#generate a pruned tree by condensing the branch reached by following "path" into a leaf
	if pathCount == length(path)
		DecisionLeaf(tree.leaf.ind, tree.leaf.prediction, tree.leaf.err)
	elseif !path[pathCount]
		DecisionLabelNode(tree.leaf, tree.splitCol, tree.leftPartition, pruneTree(tree.left, path, pathCount+1), tree.right)
	else
		DecisionLabelNode(tree.leaf, tree.splitCol, tree.leftPartition, tree.left, pruneTree(tree.right, path, pathCount+1))
	end
end

function pruneTree(tree::DecisionTestTrainLabelNode, path::Vector{Bool}, pathCount=1)
#generate a pruned tree by condensing the branch reached by following "path" into a leaf
	if pathCount == length(path)
		DecisionTestTrainLeaf(tree.leaf.ind, tree.leaf.prediction, tree.leaf.err, tree.leaf.testInd, tree.leaf.testErr)
	elseif !path[pathCount]
		DecisionTestTrainLabelNode(tree.leaf, tree.splitCol, tree.leftPartition, pruneTree(tree.left, path, pathCount+1), tree.right)
	else
		DecisionTestTrainLabelNode(tree.leaf, tree.splitCol, tree.leftPartition, tree.left, pruneTree(tree.right, path, pathCount+1))
	end
end

function pruneTree(tree::DecisionNullNode, path::Vector{Bool}, pathCount=1)
#generate a pruned tree by condensing the branch reached by following "path" into a leaf
	if pathCount == length(path)
		DecisionLeaf(tree.leaf.ind, tree.leaf.prediction, tree.leaf.err)
	elseif !path[pathCount]
		DecisionNullNode(tree.leaf, tree.splitCol, tree.splitPoint, tree.nullLeft, pruneTree(tree.left, path, pathCount+1), tree.right)
	else
		DecisionNullNode(tree.leaf, tree.splitCol, tree.splitPoint, tree.nullLeft, tree.left, pruneTree(tree.right, path, pathCount+1))
	end
end

function pruneTree(tree::DecisionNullLabelNode, path::Vector{Bool}, pathCount=1)
#generate a pruned tree by condensing the branch reached by following "path" into a leaf
	if pathCount == length(path)
		DecisionLeaf(tree.leaf.ind, tree.leaf.prediction, tree.leaf.err)
	elseif !path[pathCount]
		DecisionNullLabelNode(tree.leaf, tree.splitCol, tree.leftPartition, tree.nullLeft, pruneTree(tree.left, path, pathCount+1), tree.right)
	else
		DecisionNullLabelNode(tree.leaf, tree.splitCol, tree.leftPartition, tree.nullLeft, tree.left, pruneTree(tree.right, path, pathCount+1))
	end
end

function pruneDecisionTree(tree::DecisionTree, testInput::T, testOutput::U, previousErr = calcClassErr([predictTree(tree, v)[1] for v in testInput] .> 0.5, testOutput)) where T <: Vector{V} where V <: Union{singleNullInputType, singleInputType} where U <: DenseArray{Bool, 1} 
...
function makeRandomForest(input::T, output::U; p = 10, n = length(output), B = 100, seed = 1234) where T <: Union{vectorInputType, vectorNullInputType} where U <: DenseArray{Bool, 1}
	l = length(output)
	Random.seed!(seed)
	[begin
		bootstrapInds = ceil.(Int64, rand(n)*l)
...
function makeRandomForest(input::T, output::U, testInput, testOutput::U; p = 10, n = length(output), B = 100, seed = 1234, prune = true) where T <: Union{vectorInputType, vectorNullInputType} where U <: DenseArray{Bool, 1}
	l = length(output)
	Random.seed!(seed)
	origForest = [begin
		bootstrapInds = ceil.(Int64, rand(n)*l)
