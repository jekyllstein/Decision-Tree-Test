...
# 	# sortedSelection = sortedInd[find(view(selectedInd, sortedInd))]
# 	sortedSelection = sortedInd[find(selectedInd[sortedInd])]
...
function findRegSplit(input::U, output::Vector{T}, sqOutput::Vector{T}, selectedInd::Any, sortedInd::AbstractArray{Int64}, tmp::Array{Int64, 1}) where T <: AbstractFloat where U <: Vector{V} where V <: AbstractFloat
#finds the split point for input data to minimize the combined variance 
#of the resulting two output vectors.	
	l = length(selectedInd)
	# println(string("Selected Ind Length = ", l))
	n = length(output)
	...
	sortedSelection = if usePresort
		sortReplacementSample(sortedInd, selectedInd, tmp)
		# sortSample(sortedInd, selectedInd)
	else
		view(selectedInd, sortperm(view(input, selectedInd)))
	end
	# println(sortedSelection)
	# println(string("sq output length = ", length(sqOutput)))
	# println(string("output length = ", length(output)))
	ss = selectedSum(sqOutput, selectedInd) #sum(view(sqOutput, selectedInd))
	s1 = zero(T)
	s2 = selectedSum(output, selectedInd) #sum(view(output, selectedInd))
	# println("squared Sum = ", ss)
	# println("selected Ind Sum = ", s2)
	minValue = Inf
	minIndex = 0
	splitPoint = -Inf
	# for i in 1:l-1
	...
	# 	end
	# end
	# println(string("selectedInd = ", selectedInd))
	# println(string("length = ", l))
	for (i, j) in enumerate(sortedSelection)
		# s1 += output[selectedInd[sortedInd[i]]]
		# println(output[j])
		s1 += output[j]
		# s2 -= output[selectedInd[sortedInd[i]]]
	...
	#indtotal = collect(sortedSelection)
	ind1 = view(sortedSelection, 1:minIndex)
	ind2 = view(sortedSelection, minIndex+1:l)
	# ind1 = collect((sortedInd[q] for p in 1:tmp[sortedInd[q]]) for q in 1:minIndex)
	# ind2 = collect((sortedInd[q] for p in 1:tmp[sortedInd[q]]) for q in minIndex+1:l)
	# println(minIndex)
	# println(length(ind1))
	# println(l - minIndex)
	# println(length(ind2))
	# ind1 = (a for (i, a) in enumerate(sortedSelection) if (i <= minIndex))
	# ind2 = (a for (i, a) in enumerate(sortedSelection) if (i > minIndex))
	...
	# splitPoint = input[sortedSelection[minIndex]]
	(T(splitPoint), ind1, ind2, T(minValue), minIndex, l-minIndex)
end 
...
#finds the split point for input data to minimize the combined variance 
#of the resulting two output vectors.	
	set1 = find(view(input, selectedInd))
	set2 = find(.!view(input, selectedInd))
...
function findBoolSplit(input::DenseVector{AbstractFloat}, output::DenseVector{Bool}, sortedInd::AbstractVector{Int64}, invSortedInd::AbstractVector{Int64}, selectedInd::AbstractVector{Bool})
#finds the split point for input data to minimize the combined entropy (p_true*log2(p_true) + p_false*log2(p_false))
#of the resulting two output vectors. input is a vector of values to be split at some numerical point. 
	sortedSelection = sortedInd[find(selectedInd[sortedInd])] #selectedInd reordered in the sorting order
	l = length(sortedSelection)
	# sortedInd = sortperm(view(input, selectedInd))
...
function findBoolSplit(input::DenseVector{Int64}, output::DenseVector{Bool}, sortedInd::AbstractVector{Int64}, invSortedInd::AbstractVector{Int64}, selectedInd::AbstractVector{Bool})
#finds the split point for input data to minimize the combined entropy (p_true*log2(p_true) + p_false*log2(p_false))
#of the resulting two output vectors. input is a vector of values to be split at some numerical point. 
	sortedSelection = sortedInd[find(selectedInd[sortedInd])] #selectedInd reordered in the sorting order
	l = length(sortedSelection)
...
function findBoolSplit(input::T, output::U, selectedInd::W) where T <: floatInputType where U <: DenseArray{Bool, 1} where W <: AbstractArray{Int64, 1}
#finds the split point for input data to minimize the combined entropy (p_true*log2(p_true) + p_false*log2(p_false))
#of the resulting two output vectors. input is a vector of values to be split at some numerical point. 
	l = length(selectedInd)
	sortedInd = sortperm(view(input, selectedInd))
...
function findBoolSplit(input::T, output::U, selectedInd::W) where T <: floatNullInputType where U <: DenseArray{Bool, 1} where W <: AbstractArray{Int64, 1}
#finds the split point for input data to minimize the combined entropy (p_true*log2(p_true) + p_false*log2(p_false))
#of the resulting two output vectors. input is a vector of values to be split at some numerical point. 
	hasValues = (a.hasvalue for a in view(input, selectedInd))
	nonNullInds = find(hasValues)
	nullInds = find(a -> !a, hasValues)
	if isempty(nonNullInds)
		...
		(Nullable{AbstractFloat}(), false, Vector{Int64}(), selectedInd, AbstractFloat(minValue))
	...
	elseif length(nonNullInds) == 1
		...
		if E1 < E2
			...
			(Nullable{AbstractFloat}(), true, selectedInd, Vector{Int64}(), E1)
		else
			#better to separate nulls and non nulls
			(Nullable{AbstractFloat}(), false, view(selectedInd, nonNullInds), view(selectedInd, nullInds), E2)
		end
	else
...
function findBoolSplit(input::Vector{Int64}, output::T, selectedInd::U) where T <: DenseArray{Bool, 1} where U <: AbstractArray{Int64, 1}
#finds the split point for input data to minimize the combined entropy (p_true*log2(p_true) + p_false*log2(p_false))
#of the resulting two output vectors. input is a vector of integers representing categorical labels to be split at some
#partition of labels 
	l = length(selectedInd)
...
function findBoolSplit(input::Vector{Union{Int64, Missing}}, output::T, selectedInd::U) where T <: DenseArray{Bool, 1} where U <: AbstractArray{Int64, 1}
#finds the split point for input data to minimize the combined entropy (p_true*log2(p_true) + p_false*log2(p_false))
#of the resulting two output vectors. input is a vector of integers representing categorical labels to be split at some
#partition of labels 
	l = length(selectedInd)
...
#finds a random split point to partition input data. input is a vector of integers representing categorical labels to be split at some partition of labels 
	...
	leftPartition = Set(inputList[find(rand(Bool, length(inputList)))])
	ind1 = selectedInd[find(a -> in(a, leftPartition), view(input, selectedInd))]
	ind2 = selectedInd[find(a -> !in(a, leftPartition), view(input, selectedInd))]
	E = if isempty(ind1)
		if isempty(ind2)
			0.0
		else
...
function randomBoolSplit(input::Vector{T}, output::U, selectedInd::W) where T <: AbstractFloat where U <: DenseArray{Bool, 1} where W <: AbstractArray{Int64, 1}
#finds a random split point for input data input is a vector of values to be split at some numerical point. 
	...
	ind1 = selectedInd[find(a -> a < splitPoint, view(input, selectedInd))]
	ind2 = selectedInd[find(a -> a >= splitPoint, view(input, selectedInd))]
	E = if isempty(ind1)
		if isempty(ind2)
			0.0
		else
